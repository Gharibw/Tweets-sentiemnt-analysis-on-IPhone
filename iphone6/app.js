...
app.get('/twitterCheck', function (req, res) {
	tweeter.verifyCredentials(function (error, data) {
		res.send("Hello, " + data.name + ".  I am in your twitter.");
	});
});
...
app.post('/sentiment', function (req, res) {
	try {
		if (req.body.phrase) {
			...
			res.status(400).send('Invalid request: send {"phrase": "iPhone6Plus"}');		
		}
	} catch (exception) {
		res.status(400).send('Invalid request: send {"phrase": "iphone6Plus"}');
	}
...
app.get('/', function (req, res) {
	var welcomeResponse = "<HEAD>" +
		"<title>Sentiment Analysis of iPhone</title>\n" +
		"</HEAD>\n" +
		"<BODY>\n" +
		"<CENTER>\n" +
		"<P>\n" +
		...
		"<P>\n" +
		"<INPUT type=\"hidden\" name=\"phrase\" value=\"" + DEFAULT_TOPIC + "\"><br><br>\n" +
		"<INPUT type=\"submit\" value=\"Analyze iPhone6 feedback from Twitter\" style=\"height:100px;width:300px\">\n" +
		"</P>\n" + "</FORM>\n" + "</CENTER>\n" +"</BODY>";
	if (!monitoringPhrase) {
		...
		var monitoringResponse = "<HEAD>" +
			...
			req.headers.host +
			"/\">\n" +
			"<title>Sentiment Analysis iPhone6</title>\n" +
			"</HEAD>\n" +
			"<BODY>\n" +
			"<CENTER>\n" +
			"<P>\n" +
			"<br><br>\n<br>\nWhat do twitter users think about iPhone6?<br><br>\n \n \n \n" +
			"<IMG align=\"middle\" src=\"" + sentimentImage() + "\"/><br>\n \n <br>" +
			tweetCount + " tweets have been analyzed, and counting...<br>" +
			"</P>\n" +
...
app.get('/testSentiment', function (req, res) {
	var response = "<HEAD>" +
		"<title>Sentiment Analysis for iPhone</title>\n" +
		"</HEAD>\n" +
		"<BODY>\n" +
		"<P>\n" +
		"</P>\n" +
